...
export function createCharacterAssistantDiscoveryRequestHandler(
  discoveryService: Pick<
    iCharacterDiscoveryDirectionsService,
    'generateCharacterDiscoveryCategory'
  > = createCharacterDiscoveryDirectionsService(),
) {
  return async function handleCharacterAssistantDiscoveryRequest({ request }: { request: Request }) {
    ...
    try {
      ...
      CHARACTER_ASSISTANT_DISCOVERY_ROUTE_LOGGER.debug(
        'Character assistant discovery request accepted',
        requestContext,
      );
      const cards = await discoveryService.generateCharacterDiscoveryCategory({
...
export const handleCharacterAssistantDiscoveryRequest = createCharacterAssistantDiscoveryRequestHandler();
